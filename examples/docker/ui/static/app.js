...
fetch('/api/config')
  ...
  .then(cfg => {
    ...
    // Auto-start MAS on first load of the session
    if (!sessionStorage.getItem('dali_autostarted')) {
      sessionStorage.setItem('dali_autostarted', 'true');
      restartMas();
    }
  })
  .catch(() => {
...
function buildGrid() {
  ...
  PANES.forEach(p => {
    ...
    if (p.id !== 'server') {
      d.innerHTML +=
        `<div class="pane-ftr">` +
          `<input type="text" class="pane-input" p-id="${p.id}" ` +
                 `placeholder="Send command to ${p.id}..." ` +
                 `onkeydown="if(event.key==='Enter') sendFromPane('${p.id}')">` +
        `</div>`;
    }
...
function buildAgentSelect() {
  // Global agent select removed — we now use per-pane inputs
}
...
function sendFromPane(id) {
  const el = document.querySelector(`.pane-input[p-id="${id}"]`);
  if (!el) return;
