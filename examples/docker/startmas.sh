...
DALI_HOME=${DALI_HOME:-/dali/src}
TMUX_SOCK=${TMUX_SOCK:-/dali/shared/tmux.sock}
CONF_DIR=conf
...
tmux -S "$TMUX_SOCK" kill-session -t DALI_session 2>/dev/null || true
...
wait_for_pane() {
    ...
    while [ "$elapsed" -lt "$timeout" ]; do
        local content
        content=$(tmux -S "$TMUX_SOCK" capture-pane -pt "$target" 2>/dev/null | tr -d '[:space:]')
        [ -n "$content" ] && return 0
        sleep 0.3
        elapsed=$((elapsed + 1))
    done
...
tmux -S "$TMUX_SOCK" new-session -d -s DALI_session -n "server" \
    "$PROLOG --noinfo -l $DALI_HOME/active_server_wi.pl --goal \"go($LINDA_PORT,'server.txt').\""
...
for i in $(seq 1 150); do
    ...
    if [ $i -eq 150 ]; then
        ...
        tmux -S "$TMUX_SOCK" capture-pane -pt DALI_session:server -S -50 2>/dev/null | tail -20 | while IFS= read -r l; do log "  [server] $l"; done
...
if [ "$SPLIT_PANES" = "true" ]; then
    user_pane=$(tmux -S "$TMUX_SOCK" split-window -d -P -F '#{pane_id}' -t DALI_session:server \
        "$PROLOG --noinfo -l $DALI_HOME/active_user_wi.pl --goal utente.")
    wait_for_pane "$user_pane"
    log "END wait_for_pane user ($user_pane)"
else
    tmux -S "$TMUX_SOCK" new-window -t DALI_session -n "user" \
        "$PROLOG --noinfo -l $DALI_HOME/active_user_wi.pl --goal utente."
...
for agent_filename in "$BUILD_HOME"/*; do
    ...
    if [ "$SPLIT_PANES" = "true" ]; then
        agent_pane=$(tmux -S "$TMUX_SOCK" split-window -d -P -F '#{pane_id}' -t DALI_session:server \
        ...
        tmux -S "$TMUX_SOCK" new-window -t DALI_session -n "$agent_name" \
            "$current_dir/conf/startagent.sh $agent_base $PROLOG $DALI_HOME"
...
if [ "$SPLIT_PANES" = "true" ]; then
    tmux -S "$TMUX_SOCK" select-layout -t DALI_session:server tiled
fi
...
if [ -t 0 ]; then
    tmux -S "$TMUX_SOCK" attach -t DALI_session
    echo "Press Enter to shutdown the MAS"
    read
else
    log "Non-interactive mode: keeping script alive..."
    while tmux -S "$TMUX_SOCK" has-session -t DALI_session 2>/dev/null; do
        sleep 5
    done
fi
