...
eve(new_auction(var_N)):-retract(n_auction(var__)),assert(n_auction(var_N)),bid(var_N).

bid(var_N):-evp(alliance_accepted(var_V1)),default_bid_value(var_V2),var_V is var_V1+var_V2,agent(var_M),a(message(auctioneer,send_message(place_bid(var_V,var_M,var_N),var_M))),write(' bidded'),write(var_V),nl.

bid(var_N):-default_bid_value(var_V),agent(var_M),a(message(auctioneer,send_message(place_bid(var_V,var_M,var_N),var_M))),write(' bidded'),write(var_V),nl.

eve(alliance_accepted(var_Value)):-write('alliance ;)'),nl.

tired_to_loose:- \+evp(alliance_accepted(var_V)),n_auction(var_N),var_T1 is var_N-1,var_T2 is var_N-2,var_T3 is var_N-3,agent(var_Me),winner(var_W1,var_T1),winner(var_W2,var_T2),winner(var_W3,var_T3),\+member(var_Me,[var_W1,var_W2,var_W3]).

evi(tired_to_loose):-ally(var_A),agent(var_Me),a(message(var_A,send_message(alliance_proposal(var_Me),var_Me))),write('tired... waiting for alliance acceptance').

eve(announce_winner(var_W,var_N)):-assert(winner(var_W,var_N)).

exult(var_N):-agent(var_Me),n_auction(var_N),\+evp(exult(var_N)),winner(var_Me,var_N).

evi(exult(var_N)):-write('I won! Yeah ;)'),nl.
...
call_cfp(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_380791,var_Ontology,_380795),_380785),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_cfp(var_A,var_C,var_Ag,_380829)),a(message(var_Ag,propose(var_A,[_380829],var_AgI))),retractall(ext_agent(var_Ag,_380867,var_Ontology,_380871)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_380665,var_Ontology,_380669),_380659),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,accept_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_380735,var_Ontology,_380739)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_380553,var_Ontology,_380557),_380547),not(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,reject_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_380609,var_Ontology,_380613)).
...
call_cancel(var_A,var_Ag):-if(clause(high_action(var_A,var_Te,var_Ag),_380117),retractall(high_action(var_A,var_Te,var_Ag)),true),if(clause(normal_action(var_A,var_Te,var_Ag),_380151),retractall(normal_action(var_A,var_Te,var_Ag)),true).
...
refused_message(var_To,var_M):-clause(eliminated_message(var_M,var_To,motivation(conditions_not_verified)),_379933).
...
send_jasper_return_message(var_X,var_S,var_T,var_S0):-clause(agent(var_Ag),_379781),a(message(var_S,send_message(sent_rmi(var_X,var_T,var_S0),var_Ag))).

gest_learn(var_H):-clause(past(learn(var_H),var_T,var_U),_379729),learn_if(var_H,var_T,var_U).

evi(gest_learn(var_H)):-retractall(past(learn(var_H),_379605,_379607)),clause(agente(_379627,_379629,_379631,var_S),_379623),name(var_S,var_N),append(var_L,[46,112,108],var_N),name(var_F,var_L),manage_lg(var_H,var_F),a(learned(var_H)).

cllearn:-clause(agente(_379399,_379401,_379403,var_S),_379395),name(var_S,var_N),append(var_L,[46,112,108],var_N),append(var_L,[46,116,120,116],var_To),name(var_FI,var_To),open(var_FI,read,_379499,[]),repeat,read(_379499,var_T),arg(1,var_T,var_H),write(var_H),nl,var_T==end_of_file,!,close(_379499).
