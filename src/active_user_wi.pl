...
procedure_message(I) :-
    ...
    print('New message'),
    nl,
    print('Insert name of addressee'),
    nl,
    read(D),
    print('Insert From'),
    nl,
    ...
    read(M),
    variables(M),
    clause(result_format(F), _),
    out(message(I, D, I, From, italian, [], F)),
    nl,
    procedure_message(I).
...
variables(A, A, T, L0, L) :-
    retractall(result_format(_)),
    assert(result_format(T)),
    !,
    L = L0.
