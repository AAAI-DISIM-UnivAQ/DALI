...
chiama_ref(T,N,R,_):-go_rvar(T),
                get_value(H),unify_values(H,N,R),R\=[].
...
chiama_send_message(F,E,_,_,AgM,_,Time):-meta(E,G,AgM),go_rvar(G),
                get_value(H),extern(F,H),extern_args(F,H),
                ext_event(_,_,AgM,_,_,H,Time).
...
chiama_execute_proc(E):-go_rvar(E),
                get_value(H),functor(H,F,N),current_predicate(F/N),call(H).
...
call_meta_execute_propose(A,C,Ag):-
             clause(agent_x(_,_,F,_),_),
             once(chiama_execute_propose(F,A,C,Ag)).

chiama_execute_propose(F,A,C,Ag):-go_rvar(A),get_value(H),
                             exists_action(F,H),execute_propose(H,C,Ag).

chiama_execute_propose(F,A,C,Ag):-meta(A,G,_),go_rvar(G),
                get_value(H),exists_action(F,H),execute_propose(H,C,Ag).



%UNIFY FACTS       
unify_values(H,N,R):-findall(H,(clause(isa(H,_,_),_);clause(past(H,_,_),_)),L),
                     length(L,Lu),if(Lu<N,R=L,taglia_lista(L,N,R)).
 
% CUT LIST
taglia_lista(L,N,R):-assert(count(1)),
                     repeat,
                       clause(count(X),_),
                       nth1(X,L,El),
                       assert(list_prev(El)),
...
%TRANSFORM FROM VAR TO NAMING VAR
...
agg_liste1_rvar(L,Lu,C):-nth1(C,L,E),K is C+1, 
                         if(fdvar(E),replace_fd_rvar(L,E,Lu,K),check_new_term_rvar(L,E,Lu,K)).

%EDITED
replace_fd_rvar(L,E,Lu,K):-replace_name_rvar(E,N),substitute(E,L,N,T),agg_liste_rvar(T,Lu,K).
...
%%Proviamo EDITED
wrt_term_rvar(L):-clause(agent_x(_,_,S,_),_),name(S,Li),append(A,[46,112,108,101],Li),
...
replace_name_rvar(E,N):-atom_codes(E,Rl),append([102,100,118,97,114,95],N0,Rl),append([95,88],N0,LN),atom_codes(N,LN).

%% Fi è il nome del file "yyree.txt" blablabla 
get_value(T):-clause(name_file_app(Fi),_),
...
%CHECK PER PROPOSE
execute_propose(H,C,Ag):-verify_cond(H,C,Ag).
verify_cond(A,Lc,Ag):-Lc=[H|T],functor(H,F,N),current_predicate(F/N),call(H),!,verify_cond(A,T,Ag).
verify_cond(A,Lc,Ag):-Lc=[H|T],evp(H),!,verify_cond(A,T,Ag).
verify_cond(A,[],Ag):-assert((do_propose(A,Ag):-cd(A),asse_cosa(do_action(A,Ag)))),assert(do_action_propose(A,Ag)).
