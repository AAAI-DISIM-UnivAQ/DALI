...
%Check if temporal delta has been inserted, if yes write to pl file and assert time_add

tempo(Me):- name(Me,L), nth0(0,L,El,L1), El==116, numbertime(L1).                       %check that first character is t
numbertime(L1):- nth0(0,L1,El,L_rest),check_number(El), scorri(L_rest).                 %check if first element of list is a number and scan the list
check_number(El):- El>47, El<58.                                                        %ASCII range for numbers 0-9
scorri(L_rest):-if(L_rest=[],true, scorri_list(L_rest)).                                %check that all elements in list are numbers 
scorri_list(L_rest):- nth0(0,L_rest,X,L2), check_number(X),scorri(L2).
scrittura(Me):- name(Me,L),nth0(0,L,R,L3), append([100,101,108,116,97,116,40],L3,L1),   %write on the pl file the deltat inserted by the agent_x
...
appE(L,U):- count_external_events,length(L,N),nth1(N,L,U,R),append([101,118,101,40],R,L1), %inserito count_external_events
                re_write(L1),assert(evento_aperto).   
...
count_external_events:- clause(external_events(X),true), R is X+1, retractall(external_events(X)),assert(external_events(R)). %counter of external events

count_parentheses(El):-name(El,L),append(L,[fine],Lf),
              repeat,
                member(M,Lf),
                    if(M=40,counter_plus,true),
                    if(M=41,counter_minus,true),
                M==fine,!.

counter_plus:-clause(parentheses(X),_),R is X+1,retractall(parentheses(X)),assert(parentheses(R)).

counter_minus:-clause(parentheses(X),_),R is X-1,retractall(parentheses(X)),assert(parentheses(R)).
...
examine_clause1(Me,Nf):-if(member(Me,['(',')']), count_parentheses_le(Me),true),
		if(variabile(Me),examine_variable_le(Me,Nf),
			if(label(Me),examine_label_le(Me,Nf),write_NovarNolabel_le(Me,Nf))

		 ).

re_write1(L,Nf):-arg(1,L,N1),if(N1=39,add_39_le(L,Nf),do_not_add_le(L,Nf)).

add_39_le(L,Nf):-append([39,39,39,39,39,39],L,Lf),append(Lf,[39,39,39,39,39,39],Lf1),name(T,Lf1),open(Nf,append,Stream,[]),
                    write(Stream,T),close(Stream).
do_not_add_le(L,Nf):-name(T,L),open(Nf,append,Stream,[]),
                    write(Stream,T),close(Stream).
...
count_parentheses_le(El):-name(El,L),append(L,[fine],Lf),
              repeat,
                member(M,Lf),
                    if(M=40,counter_plus_le,true),
                    if(M=41,counter_minus_le,true),
                M==fine,!.

counter_plus_le:-clause(parentheses_le(X),_),R is X+1,retractall(parentheses_le(X)),assert(parentheses_le(R)).

counter_minus_le:-clause(parentheses_le(X),_),R is X-1,retractall(parentheses_le(X)),assert(parentheses_le(R)).
